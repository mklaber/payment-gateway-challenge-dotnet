...
public class MountebankClient(HttpClient httpClient) : IMountebankClient
{
    ...
    public async Task<PaymentResponseExternalDto> CreatePaymentAsync(PaymentRequestExternalDto paymentRequestDto, CancellationToken cancellationToken)
    {
        var resp = await httpClient.PostAsJsonAsync("payments", paymentRequestDto, MountebankJsonOptions,
            cancellationToken);
        resp.EnsureSuccessStatusCode();
        return (await resp.Content.ReadFromJsonAsync<PaymentResponseExternalDto>(MountebankJsonOptions, cancellationToken))!;
    }
}